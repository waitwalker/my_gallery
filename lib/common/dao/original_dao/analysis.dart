...
import 'package:my_gallery/model/video_record_model.dart';
...
class AnalysisDao {
  ...
  static Future<ResponseData> log(materialId, nodeId, resType, resId, {patternType}) async {
  ...
  static Future<ResponseData> report(
    ...
    var url =
        APIConst.kBaseServerURL + 'api-study-service/api/problem/records';
  ...
  static Future<ResponseData> reportVideo(
      ...
      isViewEnd,
      videoDuration}) async {
    var url = APIConst.kBaseServerURL + 'api-study-service/api/resviewlogs';
    ...
    var response =
        await NetworkManager.netFetch(url, body, null, Options(method: 'POST'));
    if (response.result) {
      var model = VideoRecordModel.fromJson(response.data);
      response.model = model;
    }
    return response;
  }
  ...
  static Future<ResponseData> reportTime({int source = 2, int? logId = 0}) async {
    var url = APIConst.kBaseServerURL + 'api-study-service/api/resviewlogs/use/log';
    ...
    var response = await NetworkManager.netFetch(url, body, null, Options(method: 'POST'));
    if (response.result) {
      var model = VideoRecordModel.fromJson(response.data);
      response.model = model;
    }
