...
StreamTransformer<String, PingInfo> _windowsTransformer = StreamTransformer.fromHandlers(
...
Future<Stream<PingInfo>> ping(String address, {int times = 4, int packetSize = 64, int interval = 1}) async {
  var process = await Process.start("ping", ["-c $times", "-s $packetSize", "-i $interval", address]);
