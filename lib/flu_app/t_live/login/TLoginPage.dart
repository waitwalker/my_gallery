...
class TLoginPage extends StatefulWidget {
  TLoginPage({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() => TLoginPageState();
}

class TLoginPageState extends State<TLoginPage> {
  ...
  login(context) async {
    ...
    await trtcVoiceRoom!.setSelfProfile('ID:' + userId, constants.DEFAULT_ROOM_IMAGE);
    if (resValue.code == 0) {
      TxUtils.showToast("Languages.of(context)!.successLogin", context);
      TxUtils.setStorageByKey(constants.USERID_KEY, userId);
      kFluRouterDelegate.push(name: FluRouterPageAPI.indexPage);
    } else {
...
// 隐藏底部输入框
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      resizeToAvoidBottomInset: false,/// 让键盘撑起内容
      appBar: AppBar(
