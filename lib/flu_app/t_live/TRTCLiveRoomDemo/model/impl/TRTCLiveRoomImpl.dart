...
class TRTCLiveRoomImpl extends LTRTCLiveRoom {
  ...
  bool isPk = false;
  int? mOriginRole;
  LTRTCLiveRoomConfig? mRoomConfig;
  ...
  @override
  Future<LActionCallback> createRoom(int? roomId, LRoomParam roomParam) async {
    if (!mIsLogin) {
      return LActionCallback(
    ...
    return LActionCallback(code: code, desc: msg);
  }

  @override
  Future<LActionCallback> destroyRoom() async {
    V2TimCallback dismissRes = await timManager.dismissGroup(groupID: mRoomId!);
    if (dismissRes.code == 0) {
      _destroyData();
      await mTRTCCloud!.exitRoom();
      return LActionCallback(code: 0, desc: "dismiss room success.");
    } else {
      return LActionCallback(code: codeErr, desc: "dismiss room fail.");
    }
  }
  ...
  @override
  Future<LActionCallback> enterRoom(int? roomId) async {
    if (mIsEnterRoom) {
      return LActionCallback(
          code: codeErr,
          desc: "you have been in room:" +
    ...
    return LActionCallback(code: joinRes.code, desc: joinRes.desc);
  }

  @override
  Future<LActionCallback> exitRoom() async {
    if (mRoomId == null) {
      return LActionCallback(code: codeErr, desc: "not enter room yet");
    }
    _destroyData();
    await mTRTCCloud!.exitRoom();
    ...
    if (quitRes.code != 0) {
      return LActionCallback(code: codeErr, desc: quitRes.desc);
    }

    return LActionCallback(code: 0, desc: "quit room success.");
  }

  @override
  Future<LUserListCallback> getAnchorList() async {
    V2TimValueCallback<List<V2TimUserFullInfo>> res =
        await timManager.getUsersInfo(userIDList: mAnchorList as List<String>);

    if (res.code == 0) {
      List<V2TimUserFullInfo> userInfo = res.data!;
      List<LUserInfo> newInfo = [];
      for (var i = 0; i < userInfo.length; i++) {
        newInfo.add(LUserInfo(
            userId: userInfo[i].userID,
            userName: userInfo[i].nickName,
            userAvatar: userInfo[i].faceUrl));
      }
      return LUserListCallback(
          code: 0, desc: 'get archorInfo success.', list: newInfo);
    } else {
      return LUserListCallback(code: res.code, desc: res.desc);
    }
  }

  @override
  Future<LUserListCallback> getRoomMemberList(int nextSeq) async {
    ...
    if (memberRes.code != 0) {
      return LUserListCallback(code: memberRes.code, desc: memberRes.desc);
    }
  ...
  @override
  Future<LRoomInfoCallback> getRoomInfos(List<String?> roomIdList) async {
    print("==roomIdList=" + roomIdList.toString());

    V2TimValueCallback<List<V2TimGroupInfoResult>> res = await timManager
        .getGroupManager()
        .getGroupsInfo(groupIDList: roomIdList as List<String>);
    if (res.code != 0) {
      return LRoomInfoCallback(code: res.code, desc: res.desc);
    }

    List<V2TimGroupInfoResult> listInfo = res.data!;

    List<LRoomInfo> newInfo = [];
    for (var i = 0; i < listInfo.length; i++) {
      print(listInfo[i].toJson());
      if (listInfo[i].resultCode == 0) {
        //兼容获取不到群id信息的情况
        V2TimGroupInfo groupInfo = listInfo[i].groupInfo!;
        newInfo.add(LRoomInfo(
    ...
    return LRoomInfoCallback(
        code: 0, desc: 'getRoomInfoList success', list: newInfo);
  }

  @override
  Future<LActionCallback> kickoutJoinAnchor(String userId) async {
    V2TimValueCallback res = await timManager.getSignalingManager().invite(
        invitee: userId,
        data: jsonEncode(_getCustomMap(kickoutAnchorCMD)),
        timeout: 0,
        onlineUserOnly: false);
    return LActionCallback(code: res.code, desc: res.desc);
  }

  @override
  Future<LActionCallback> login(int sdkAppId, String userId, String userSig,
      LTRTCLiveRoomConfig config) async {
    mSdkAppId = sdkAppId;
    mUserId = userId;
    mUserSig = userSig;
    mRoomConfig = config;
    ...
    if (!mIsInitIMSDK) {
      ...
      if (initRes.code != 0) {
        //初始化sdk错误
        return LActionCallback(code: 0, desc: 'init im sdk error');
      }
    }
    mIsInitIMSDK = true;
    ...
    if (loginedUserId != null && loginedUserId == userId) {
      mIsLogin = true;
      return LActionCallback(code: 0, desc: 'login im success');
    }
    ...
    if (loginRes.code == 0) {
      ...
      return LActionCallback(code: codeErr, desc: loginRes.desc);
    }
  }

  @override
  Future<LActionCallback> logout() async {
    mSdkAppId = 0;
    mUserId = "";
    mUserSig = "";
    mIsLogin = false;
    V2TimCallback loginRes = await timManager.logout();
    return LActionCallback(code: loginRes.code, desc: loginRes.desc);
  }
  ...
  @override
  Future<LActionCallback> quitRoomPK() async {
    V2TimValueCallback res = await timManager.getSignalingManager().invite(
        invitee: mUserIdPK,
  ...
  @override
  Future<LActionCallback> requestJoinAnchor() async {
    V2TimValueCallback res = await timManager.getSignalingManager().invite(
  ...
  @override
  Future<LActionCallback> requestRoomPK(int roomId, String userId) async {
    if (mAnchorList.length >= 2) {
      return LActionCallback(
          code: codeErr,
          desc:
  ...
  @override
  Future<LActionCallback> responseJoinAnchor(
  ...
  @override
  Future<LActionCallback> responseRoomPK(String userId, bool agree) async {
    V2TimCallback res;
    if (agree) {
      res = await timManager.getSignalingManager().accept(
  ...
  @override
  Future<LActionCallback> sendRoomTextMsg(String message) async {
    ...
    if (res.code == 0) {
      return LActionCallback(code: 0, desc: "send group message success.");
    } else {
      return LActionCallback(
  ...
  @override
  Future<LActionCallback> sendRoomCustomMsg(String cmd, String message) async {
    V2TimValueCallback<V2TimMessage> res =
  ...
  @override
  Future<LActionCallback> setSelfProfile(
    ...
    if (res.code == 0) {
      return LActionCallback(code: 0, desc: "set profile success.");
    } else {
      return LActionCallback(code: res.code, desc: "set profile fail.");
    }
  }
  ...
  Future<void> updateRemoteView(String userId, int viewId) {
    return mTRTCCloud!.updateRemoteView(
        viewId.toString(), TRTCCloudDef.TRTC_VIDEO_STREAM_TYPE_BIG, int.parse(userId));
  }

  @override
  Future<LActionCallback> startPublish(String streamId) async {
    if (!mIsEnterRoom) {
      return LActionCallback(code: codeErr, desc: "not enter room yet.");
    }
    // 如果是观众，那么则切换到主播
    ...
    return LActionCallback(code: 0, desc: "startPublish success");
  }
