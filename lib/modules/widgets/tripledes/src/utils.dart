...
concat(List<int?> a, List<int> b) {
  ...
  if (thisSigBytes % 4 != 0) {
    ...
    for (var i = 0; i < thatSigBytes; i++) {
      ...
      if (thisWords[idx] != null) {
        thisWords[idx] = thisWords[idx]! | thatByte << (24 - ((thisSigBytes + i) % 4) * 8);
      }
    }
  } else {
...
void expandList(List<int?> data, int newLength) {
  ...
  // replace any allocations with 0
...
void clamp(List<int?> data) {
  ...
  // Clamp
  if (words[rightShift32(sigBytes, 2)] != null) {
    words[rightShift32(sigBytes, 2)] = words[rightShift32(sigBytes, 2)]! & (0xffffffff << (32 - (sigBytes % 4) * 8)).toSigned(32);
  }
