...
/// 播放器控件，带菜单蒙层
class MenuMaterialControls extends StatefulWidget {
  final String? title;

  MenuMaterialControls({Key? key, this.title}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _MenuMaterialControlsState();
  }
}

class _MenuMaterialControlsState extends State<MenuMaterialControls> {
  ...
  late Duration durationCanDrag;

  bool _inDragH = false;
  bool _inDragL = false;
  bool _inDragR = false;

  bool get inDrag => _inDragH || _inDragL || _inDragR;
  ...
  Widget _buildView(BuildContext context) {
    ...
    if (_showPlayerMenu)
      return AnimatedOpacity(
          ...
          opacity: 1,
          child: InkWell(
              child: PlayerMenu(memoizer: _memoizer),
              onTap: () {
                setState(() {
        ...
        child: Row(
          ...
          children: <Widget>[
            (chewieController!.isFullScreen)
                ...
                : Container(width: 20),
            Container(
                width: 280, alignment: Alignment.center, child: _buildTitle()),
  ...
  void _updateState() {
    // if in drag state, ignore
    if (!inDrag) {
      setState(() {
        _latestValue = controller!.value;
        durationCanDrag = Duration(
            seconds: (controller!.value.duration?.inSeconds ?? 60) ~/ 3);
      });
    }
  }
