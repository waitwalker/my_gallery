...
class MaterialControls extends StatefulWidget {
  final String? title;
  final bool isLocalPlay;

  MaterialControls({Key? key, this.title, this.isLocalPlay = false}) : super(key: key);
...
class _MaterialControlsState extends State<MaterialControls> {
    ...
    if (widget.isLocalPlay) {
      ...
      return <Widget>[
        (chewieController!.isFullScreen)
            ? _buildBack(controller)
            : Container(width: 20),
        builderTitleContent(chewieController!.isFullScreen),
  ...
  builderTitleContent(bool isFullScreen) {
    if (!isFullScreen) {
      return Container(
  ...
  _buildTitle() {
    print("标题名称:${widget.title}");
    return Text(widget.title ?? '', style: TextStyle(color: Colors.white, fontSize: 17), overflow: TextOverflow.ellipsis,);
  }
