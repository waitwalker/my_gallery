...
import 'package:my_gallery/modules/login/city_pickers/modal/point.dart';
...
//    return Point.nullPoint;
  ...
  Point _buildTree(Point target, Map? cities, Map? meta) {
    if (cities == null || cities.isEmpty) {
      return target;
    } else {
      List<String> keys = cities.keys.toList() as List<String>;

      for (int i = 0; i < keys.length; i++) {
        String key = keys[i];
        Map value = cities[key];
        ...
        //        }
        //    }
        if (cities.keys.length == 1) {
          if (target.code.toString() == cities.keys.first) {
            continue;
          }
        }
