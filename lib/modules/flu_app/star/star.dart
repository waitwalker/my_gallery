...
/// 回调
typedef StarsChanged = void Function(double starCount, double selectedStars);
...
class Stars extends StatefulWidget {
  ...
  /// 数值发生变化的回调
  final StarsChanged? starsChanged;
...
class _StarsState extends State<Stars> {
  ...
  void initState() {
    ...
    setupRealStars(_currentStars, false, false, false);
  }

  @override
  void didUpdateWidget(Stars oldWidget) {
      ...
      children: [
        Listener(
          onPointerDown: (event) {
            ...
            calculateSelectedStars(event.localPosition.dx, false);
          },
          onPointerMove: (event) {
            if (widget.onlyShow) {
              return;
          ...
          onPointerUp: (event) {
            ...
            calculateSelectedStars(event.localPosition.dx, true);
          },
          child: Row(
        ...
        IgnorePointer(
          child: ClipRect(
            clipper: FFStarsClipper(getRealWidth()),
            child: Row(
              mainAxisSize: MainAxisSize.min,
  ...
  void calculateSelectedStars(double width, bool needCallback) {
    ...
    /// 4.计算实际得分
    setupRealStars(selectedStars, true, true, needCallback);
  }
