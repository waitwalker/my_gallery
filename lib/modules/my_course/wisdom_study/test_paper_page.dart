...
import 'package:my_gallery/modules/my_course/wisdom_study/exam_mode_prompt_page.dart';
...
class TestPaperPage extends StatefulWidget {
  ...
  TestPaperPage(this.data, {this.courseId, this.task, this.level, this.materialid, this.nodeid, this.isdiagnosis});
            ...
            onTap: () {
              Navigator.of(context).push(MaterialPageRoute(builder: (_) {
                return ExerciseRecordPage(
                    widget.data.resId,
                    type: 2,
                    srcAbPaperId: widget.data.srcABPaperQuesIds);
              }));
            },
          ),
          top: 15,
...
//                                    return ExamModePromptPage(widget.data,widget.courseId,widget.data.resName);
//                                  }));
//                                },
//                              ),
//                            ),
                              ...
                              child: ElevatedButton(
                                ...
                                onPressed: () {
                                  Navigator.of(context).push(MaterialPageRoute(builder: (_) {
                                    ...
                                    var url;
                                    if (widget.task == null) {
                                      url = '${APIConst.practiceHost}/ab.html?token=$token&abpid=$abpid&abpname=$abpname&abpqids=$abpqids&courseid=${widget.courseId}';
                                      if (widget.nodeid != null) {
                                        url = '${APIConst.practiceHost}/ab.html?token=$token&abpid=$abpid&abpname=$abpname&abpqids=$abpqids&courseid=${widget.courseId}&materialid=${widget.materialid}&nodeid=${widget.nodeid}&level=${widget.level}';
                                      }
                                    } else {
                                      url = '${APIConst.practiceHost}/ab.html?token=$token&abpid=$abpid&abpname=$abpname&abpqids=$abpqids&taskid=${widget.task!.taskId}&courseid=${widget.task!.cardCourseId ?? 0}';
                                    }
